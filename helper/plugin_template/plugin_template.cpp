...
void plugin_template::run() {
  ...
  parser.addPositionalArgument(
      "tamplate name",
      QCoreApplication::translate("main", "name for the template"));
  parser.addPositionalArgument(
      "destination",
  ...
  if (!path.mkdir(name.toLower())) {
    qWarning() << QCoreApplication::translate("main", "Can't create directory ")
               << name.toLower();
    quit();
  }
  ...
  if (file->open(QFile::WriteOnly | QFile::Text)) {
    ...
    *ts << saveHeader(name);
    ...
    qWarning() << QCoreApplication::translate("main", "Can't create ")
               << name.toLower() << ".hpp";
  }
...
QString plugin_template::saveCMakeLists(const QString &name) {
  ...
  str += "\n";
  str += "set(CMAKE_MODULE_PATH ${CMAKE_MODULE_PATH} "
         "\"${CMAKE_CURRENT_SOURCE_DIR}/CMakeModules/\")\n";
  str += "\n";
  str += "find_package(Qt5Core)\n";
  str += "find_package(Qt5Gui)\n";
  str += "\n";
  str += "include_directories(${Qt5Core_INCLUDE_DIRS} ${Qt5Gui_INCLUDE_DIRS} "
  ...
  str += ")\n";
  str += "\n";
  str += "qt5_add_resources(" + name.toLower() + "_QRC " + name.toLower() +
         ".qrc)\n";
  str += "\n";
  str += "add_library(" + name.toLower() + " SHARED ${" + name.toLower() +
         "_SRC} ${" + name.toLower() + "_QRC})\n";
  str += "target_link_libraries(" + name.toLower() +
         " ${Qt5Core_LIBRARIES} ${Qt5Gui_LIBRARIES})\n";
  str += "\n";
  str += "qt5_use_modules(" + name.toLower() + " Core Gui)\n";
  str += "\n";
  str += "install(TARGETS " + name.toLower() +
...
QString plugin_template::saveCpp(const QString &name) {
  ...
  str += "/*\n";
  str += " * <one line to give the program's name and a brief idea of what it "
         "does.>\n";
  str += " * Copyright (C) 2014  <copyright holder> <email>\n";
  str += " *\n";
  str += " * This program is free software: you can redistribute it and/or "
         "modify\n";
  str += " * it under the terms of the GNU General Public License as published "
         "by\n";
  str +=
      " * the Free Software Foundation, either version 3 of the License, or\n";
  ...
  str +=
      " * You should have received a copy of the GNU General Public License\n";
  str += " * along with this program.  If not, see "
  ...
  str += "\n";
  str += "	isActive = false;\n";
  str += "	screen = new gScreen(QImage(\":/" + name.toLower() +
         "/icon.png\"), tr(\"Hello World\"));\n";
  str += "}\n";
  str += "\n";
  str += "" + name + "::~" + name + "()\n";
  str += "{\n";
...
QString plugin_template::saveHeader(const QString &name) {
  ...
  str += "\n";
  str += "	signals:\n";
  str += "		void doAction(gAction action, void *data);	"
         "		// Signal to draw img on screen\n";
  str += "		\n";
  str += "};\n";
  str += "\n";
