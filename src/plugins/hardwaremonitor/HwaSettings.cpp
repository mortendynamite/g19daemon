...
HwaSettings::HwaSettings()
        : generalSettings_(GeneralSettings{TemperatureType::Celsius}) {}

HwaSettings::~HwaSettings() { removeSettings(); }
...
void HwaSettings::loadSettings() {
    ...
    for (int i = 0; i < screenTotal; i++) {
        ...
        ScreenType type =
                Defines::translateScreenTypeEnum(settings_->value("type").toString());

        if (type == ScreenType::Normal) {
            loadNormalScreenSettings(name, background, type);

            QList<CustomSettings> list = loadCustomSettings();
            NormalScreen *currentScreen =
                    static_cast<NormalScreen *>(getScreenData(name));
    ...
    qDebug() << "Total screens loaded from settings: " +
                QString::number(screenList_.size()) + " of " +
...
void HwaSettings::loadNormalScreenSettings(QString name, QString background,
                                           ScreenType type) {
    ...
    for (int j = 0; j < totalLines; j++) {
        ...
        for (int k = 0; k < totalData; k++) {
            ...
            query.system = Defines::translateMonitorSystemEnum(
                    settings_->value("system").toString());
            query.identifier = settings_->value("id").toString();
            query.name = settings_->value("name").toString();
            query.value = Defines::translateQueryValueEnum(
                    settings_->value("value").toString());
...
void HwaSettings::loadGraphScreenSettings(QString name, QString background,
                                          ScreenType type) {
    ...
    for (int i = 0; i < graphCount; i++) {
        ...
        query.name = settings_->value("name").toString();
        query.value =
                Defines::translateQueryValueEnum(settings_->value("value").toString());
        query.precision = settings_->value("precision").toInt();
        query.field = settings_->value("field").toString();
        query.unit = settings_->value("unit").toString();
        query.hardware = settings_->value("hardware").toString();
...
void HwaSettings::changeScreenOrder(QList<QString> mainOrder,
                                    QMap<QString, QList<QString>> subOrder) {
    mainOrder_.clear();
    subOrder_.clear();
...
void HwaSettings::loadGeneralSettings() {
    ...
    generalSettings_.temperature = Defines::translateTemperatureEnum(
            settings_->value("General/Temperature").toString());
    ...
    influxSettings.username =
            settings_->value("Influx/influxUsername").toString();
    influxSettings.password =
            settings_->value("Influx/influxPassword").toString();
    influxSettings.hostname =
            settings_->value("Influx/influxHostname").toString();
    influxSettings.database =
            settings_->value("Influx/influxDatabase").toString();

    generalSettings_.influxDbSettings = influxSettings;
}

void HwaSettings::creategraphScreen(QString name, QString background,
                                    ScreenType type, QList<GraphLine> graphData,
...
void HwaSettings::createNormalScreen(QString name, QString background,
                                     ScreenType type, QList<LineText> lines) {
...
QList<CustomSettings> HwaSettings::loadCustomSettings() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        custom.aligment =
                Defines::translateAligmentEnum(settings_->value("Aligment").toString());
...
QString HwaSettings::getLanguage() { return generalSettings_.language; }

InfluxDbSettings HwaSettings::getInfluxSettings() {
    return generalSettings_.influxDbSettings;
}

QVector<Screen *> HwaSettings::getScreenList() { return screenList_; }

QList<LineText> HwaSettings::optimizeData(QStringList lines,
                                          QMap<QString, Query> dataList) {
    QList<LineText> data;
...
QList<Screen *> HwaSettings::getMainOrder() { return mainOrder_; }

QMap<QString, QList<Screen *>> HwaSettings::getSubOrder() { return subOrder_; }
