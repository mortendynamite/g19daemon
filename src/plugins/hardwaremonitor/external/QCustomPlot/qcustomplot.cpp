...
/*! \class QCPScatterStyle
  ...
  functions on an instance: \code QCPScatterStyle myScatter;
...
/*!
  ...
  If the item is represented not by single lines but by an area like QCPItemRect
  or QCPItemText, a click inside the area returns a constant value greater zero
  (typically the selectionTolerance of the parent QCustomPlot multiplied by
  0.99). If the click lies outside the area, this function returns -1.0.
...
/*! \class QCPMarginGroup
  ...
  left margins with each other and their right margins with each other.
*/
...
QCPLayoutElement::~QCPLayoutElement() {
  ...
  if (qobject_cast<QCPLayout *>(
          mParentLayout)) // the qobject_cast is just a safeguard in case the
                          // layout forgets to call clear() in its dtor and this
                          // dtor is called by QObject dtor
...
/*!
  ...
  layout and then deletes the returned element.
...
/*!
  ...
  This function internally uses \ref takeAt to remove the element from the
  layout and then deletes the element.
    ...
    if (foundMinimumViolation) {
      ...
      for (int i = 0; i < sectionCount; ++i) {
        if (!minimumLockedSections.contains(
          ...
          freeSize -=
              sectionSizes.at(i); // remove size of minimum locked sections from
...
/*! \class QCPLineEnding
  ...
  expected, e.g. \code myItemLine->setHead(QCPLineEnding::esSpikeArrow) \endcode
*/
  ...
  if (haveRange) {
                       ...
                       // range to at least center the plottable
    {
      double center =
          ...
          0.5; // upper and lower should be equal anyway, but just to make sure,
...
double QCPAxis::coordToPixel(double value) const {
  if (orientation() == Qt::Horizontal) {
    ...
    {
      if (value >= 0 &&
          mRange.upper < 0) // invalid value for logarithmic scale, just draw it
...
void QCPAxis::draw(QCPPainter *painter) {
  ...
  QVector<double> subTickPositions; // the final coordToPixel transformed vector
...
void QCPAxisPainterPrivate::draw(QCPPainter *painter) {
  ...
  double xCor = 0, yCor = 0; // paint system correction, for pixel exact matches
  ...
  painter->setAntialiasing(true); // always want endings to be antialiased, even
                                  // if base and ticks themselves aren't
  ...
  QSize tickLabelsSize(
      0, 0); // size of largest tick label, for offset calculation of axis label
...
void QCPAxisPainterPrivate::placeTickLabel(QCPPainter *painter, double position,
          ...
          QCPPainter::pmNoCaching)) // label caching enabled
  {
    if (!mLabelCache.contains(text)) // no cached label exists, create it
    {
      ...
      newCachedLabel->offset = getTickLabelDrawOffset(labelData) +
...
QCPAxisPainterPrivate::getTickLabelData(const QFont &font,
  ...
  if (result.baseFont.pointSizeF() >
      0) // On some rare systems, this sometimes is initialized with -1 (Qt
         // bug?), so we check here before possibly setting a negative value in
         // the next line
    result.baseFont.setPointSizeF(
        result.baseFont.pointSizeF() +
        0.05); // QFontMetrics.boundingRect has a bug for exact point sizes that
               // make the results oscillate due to internal rounding
  if (useBeautifulPowers) {
    ...
    result.totalBounds = result.baseBounds.adjusted(
        ...
        0); // +2 consists of the 1 pixel spacing between base and exponent (see
            // drawTickLabel) and an extra pixel to include AA
...
/*!
  ...
  For example, the \ref QCPGraph subclass draws its graph lines and scatter
  points with this pen.
...
/*! \class QCPAbstractItem
  ...
  we need to change the position type like this: \code
  ...
  the axis rect: \code line->setClipToAxisRect(false); \endcode
...
/*! \internal
  ...
  return the minimum of all four returned values which were greater or equal to
  zero. (Because this function may return -1.0 when \a pos doesn't hit \a rect
  at all). If all calls returned -1.0, return -1.0, too, because your item
  wasn't hit.
...
QCPAbstractPlottable *QCustomPlot::plottableAt(const QPointF &pos,
  ...
  foreach (QCPAbstractPlottable *plottable, mPlottables) {
    if (onlySelectable &&
        !plottable->selectable()) // we could have also passed onlySelectable to
...
bool QCustomPlot::removeLayer(QCPLayer *layer) {
  ...
  if (isFirstLayer) // prepend in reverse order (so order relative to each other
                    // stays the same)
  {
...
void QCustomPlot::replot(QCustomPlot::RefreshPriority refreshPriority) {
  ...
  if (painter.isActive()) {
    painter.setRenderHint(
        QPainter::HighQualityAntialiasing); // to make Antialiasing look good if
...
void QCustomPlot::mouseReleaseEvent(QMouseEvent *event) {
      ...
      5) // determine whether it was a click operation
  {
    ...
    QCPLayerable *clickedLayerable = layerableAt(
        event->pos(), false,
        &details); // for these signals, selectability is ignored, that's why we
                   // call this again with onlySelectable set to false
    if (QCPAbstractPlottable *ap =
            qobject_cast<QCPAbstractPlottable *>(clickedLayerable))
...
void QCustomPlot::toPainter(QCPPainter *painter, int width, int height) {
  ...
  if (painter->isActive()) {
    ...
    setViewport(QRect(0, 0, newWidth, newHeight));
    painter->setMode(QCPPainter::pmNoCaching);
    // warning: the following is different in toPixmap, because a solid
    // background color is applied there via QPixmap::fill here, we need to do
    // this via QPainter::fillRect.
    if (mBackgroundBrush.style() != Qt::NoBrush)
      painter->fillRect(mViewport, mBackgroundBrush);
    draw(painter);
    setViewport(oldViewport);
...
/*! \class QCPColorGradient
  ...
  e.g.: \code colorMap->setGradient(QCPColorGradient::gpHot); \endcode
    ...
    if (it.value().contains(axis)) {
      ...
      if (qobject_cast<QCustomPlot *>(
              parentPlot())) // make sure this isn't called from QObject dtor
                             ...
                             // when the axis rect is not in any layout and thus
...
void QCPAxisRect::drawBackground(QCPPainter *painter) {
  ...
  if (!mBackgroundPixmap.isNull()) {
    if (mBackgroundScaled) {
      ...
      painter->drawPixmap(mRect.topLeft(), mBackgroundPixmap,
                          QRect(0, 0, mRect.width(), mRect.height()));
    }
  }
}
...
void QCPAxisRect::updateAxesOffset(QCPAxis::AxisType type) {
  ...
  for (int i = 1; i < axesList.size(); ++i) {
    ...
    if (axesList.at(i)
            ->visible()) // only add inner tick length to offset if this axis is
                         // visible and it's not the first visible one (might
                         // happen if true first axis is invisible)
    {
      if (!isFirstVisible)
...
void QCPAxisRect::mousePressEvent(QMouseEvent *event) {
  mDragStart =
      event->pos(); // need this even when not LeftButton is pressed, to
                    // determine in releaseEvent whether it was a full click (no
                    // position change between press and release)
...
QCPLegend::~QCPLegend() {
  ...
  if (qobject_cast<QCustomPlot *>(
          mParentPlot)) // make sure this isn't called from QObject dtor when
                        ...
                        // legend is not in any layout and thus QObject-child of
                        // QCustomPlot)
        ...
        additive ? mSelectedParts ^ spLegendBox
                 : mSelectedParts |
                       spLegendBox); // no need to unset spItems in !additive
                                     // case, because they will be deselected by
...
/*! \class QCPPlotTitle
  ...
  \code
  customPlot->plotLayout()->insertRow(0); // inserts an empty row above the
  default axis rect customPlot->plotLayout()->addElement(0, 0, new
  QCPPlotTitle(customPlot, "Your Plot Title")); \endcode
...
/*! \class QCPColorScale
  ...
  automatically generated ticks, call \code
  colorScale->axis()->setAutoTickCount(3);
  \endcode

  Placing a color scale next to the main axis rect works like with any other
  layout element: \code QCPColorScale *colorScale = new
  QCPColorScale(customPlot); customPlot->plotLayout()->addElement(0, 1,
  colorScale); colorScale->setLabel("Some Label Text"); \endcode In this case we
  have placed it to the right of the default axis rect, so it wasn't necessary
  to call \ref setType, since \ref QCPAxis::atRight is already the default. The
  text next to the color scale can be set with \ref setLabel.
  ...
  to achieve this: \code QCPMarginGroup *group = new QCPMarginGroup(customPlot);
...
QCPColorScale::QCPColorScale(QCustomPlot *parentPlot)
    : QCPLayoutElement(parentPlot),
      mType(QCPAxis::atTop), // set to atTop such that setType(QCPAxis::atRight)
                             // below doesn't skip work because it thinks it's
                             // already atRight
...
void QCPColorScale::setType(QCPAxis::AxisType type) {
  ...
  if (mType != type) {
    ...
    foreach (QCPAxis::AxisType atype, QList<QCPAxis::AxisType>()
    ...
    */
    ...
    if (mParentPlot->plottingHints().testFlag(QCP::phFastPolylines) &&
      ...
      painter->drawPolyline(lineData->constData() + segmentStart,
                            lineDataSize -
                                segmentStart); // lineDataSize, because we do
                                               // want to include the last point
    }
  }
}
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData,
  ...
  if (mAdaptiveSampling &&
      dataCount >= maxCount) // use adaptive sampling only if there are at least
    ...
    if (lineData) {
      ...
      while (it != upperEnd) {
        if (it.key() < currentIntervalStartKey +
                           keyEpsilon) // data point is still within same pixel,
                                       // so skip it and expand value span of
                                       // this cluster if necessary
        {
...
/*! \internal
  ...
  representation of the graph), returns 500.
*/
double QCPGraph::pointDistance(const QPointF &pixelPoint) const {
  ...
  if (mLineStyle == lsNone) {
    ...
    QPointF ptB = coordsToPixels(
        ...
        scatterData->at(0).value); // getScatterPlotData returns in plot
                                   // coordinates, so transform to pixels
    ...
    double minDistSqr = std::numeric_limits<double>::max();
    if (mLineStyle == lsImpulse) {
      // impulse plot differs from other line styles in that the lineData points
...
QCPRange QCPGraph::getKeyRange(bool &foundRange, SignDomain inSignDomain,
             ...
             sdNegative) // range may only be in the negative sign domain
  {
    ...
    while (it != mData->constEnd()) {
      ...
      if (includeErrors) // in case point is in valid sign domain but errobars
                         // stretch beyond it, we still want to geht that point.
      {
             ...
             sdPositive) // range may only be in the positive sign domain
  {
    ...
    while (it != mData->constEnd()) {
      ...
      if (includeErrors) // in case point is in valid sign domain but errobars
                         // stretch beyond it, we still want to get that point.
      {
        if ((current < range.lower || !haveLower) && current > 0) {
          range.lower = current;
          haveLower = true;
...
/*! \class QCPCurve
  ...
  \code
  QCPCurve *newCurve = new QCPCurve(customPlot->xAxis,
  customPlot->yAxis);\endcode add it to the customPlot with
  QCustomPlot::addPlottable: \code customPlot->addPlottable(newCurve);\endcode
...
void QCPCurve::getCurveData(QVector<QPointF> *lineData) const {
  ...
  QVector<QPointF>
      trailingPoints; // points that must be applied after all other points (are
             ...
             // this point at original position
      {
        if (it ==
            mData->constBegin()) // it is first point in curve and prevIt is
                                 // last one. So save optimized point for adding
                                 // it to the lineData in the end
...
QPointF QCPCurve::getOptimizedPoint(int otherRegion, double otherKey,
  ...
  case 5: {
    break; // case 5 shouldn't happen for this function but we add it anyway to
           // prevent potential discontinuity in branch table
  }
  case 6: // bottom edge
  {
    intersectValue = rectBottom;
...
QVector<QPointF> QCPCurve::getOptimizedCornerPoints(
  ...
  case 1: {
    ...
    case 9: { // in this case we need another distinction of cases: segment may
              // pass below or above rect, requiring either bottom right or top
              // left corner points
  ...
  case 3: {
    ...
    case 7: { // in this case we need another distinction of cases: segment may
              // pass below or above rect, requiring either bottom right or top
              // left corner points
  ...
  case 7: {
    ...
    case 3: { // in this case we need another distinction of cases: segment may
              // pass below or above rect, requiring either bottom right or top
              // left corner points
      if ((value - prevValue) / (key - prevKey) * (rectRight - key) + value <
          rectBottom) // segment passes below R
  ...
  case 9: {
    ...
    case 1: { // in this case we need another distinction of cases: segment may
              // pass below or above rect, requiring either bottom right or top
              // left corner points
      if ((value - prevValue) / (key - prevKey) * (rectLeft - key) + value <
          rectBottom) // segment passes below R
...
/*! \class QCPBarsGroup
  ...
  respective bars intances: \code QCPBarsGroup *group = new
  QCPBarsGroup(customPlot); group->append(bars1); group->append(bars2); \endcode
...
/*! \class QCPStatisticalBox
  ...
  Usually, you first create an instance:
  \code
  QCPStatisticalBox *newBox = new QCPStatisticalBox(customPlot->xAxis,
  customPlot->yAxis);\endcode add it to the customPlot with
  QCustomPlot::addPlottable: \code customPlot->addPlottable(newBox);\endcode and
  then modify the properties of the newly created plottable, e.g.: \code
...
/*! \class QCPColorMap
  ...
  Usually, you first create an instance:
  \code
  QCPColorMap *colorMap = new QCPColorMap(customPlot->xAxis,
  customPlot->yAxis);\endcode add it to the customPlot with
  QCustomPlot::addPlottable: \code customPlot->addPlottable(colorMap);\endcode
...
void QCPColorMap::draw(QCPPainter *painter) {
  ...
  if (mMapData->keySize() > 1)
    halfSampleKey =
        0.5 * mMapData->keyRange().size() / (double)(mMapData->keySize() - 1);
  if (mMapData->valueSize() > 1)
    halfSampleValue = 0.5 * mMapData->valueRange().size() /
                      (double)(mMapData->valueSize() - 1);
  QRectF imageRect(
      coordsToPixels(mMapData->keyRange().lower - halfSampleKey,
  ...
  bool mirrorY =
      (valueAxis()->orientation() == Qt::Vertical ? valueAxis() : keyAxis())
          ->rangeReversed();
  bool smoothBackup =
      painter->renderHints().testFlag(QPainter::SmoothPixmapTransform);
  painter->setRenderHint(QPainter::SmoothPixmapTransform, mInterpolate);
  QRegion clipBackup;
  if (mTightBoundary) {
    clipBackup = painter->clipRegion();
    painter->setClipRect(QRectF(coordsToPixels(mMapData->keyRange().lower,
  ...
  } else if (qFuzzyIsNull(vec.y()))  // line is horizontal
  {
    ...
    if (gamma >= 0 && gamma <= rect.height())
      result.setLine(rect.left(), by + gamma, rect.right(),
                     by + gamma); // no need to check right because we know line
                                  // is horizontal
  } else                          // line is skewed
  {
...
void QCPItemCurve::draw(QCPPainter *painter) {
  ...
  if (cubicRect.isEmpty()) // may happen when start and end exactly on same x or
                           // y position
