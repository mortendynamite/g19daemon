...
Mpris::Mpris() {
  ...
  connect(mpris, SIGNAL(statusChanged(PlayerStatus)), this,
          SLOT(onStatusChanged(PlayerStatus)));
  connect(mpris, SIGNAL(trackChanged(MediaData)), this,
          SLOT(onTrackChanged(MediaData)));
...
QStringList Mpris::getPlayersList() {
  ...
  QStringList services = QDBusConnection::sessionBus()
                             .interface()
                             ->registeredServiceNames()
                             .value()
                             .filter(QLatin1String("org.mpris."));
...
void Mpris::paint() {
  ...
  if (!albumArt->isNull()) {
    QImage tmp;
    tmp = albumArt->scaled(320, 206, Qt::KeepAspectRatio,
...
bool Mpris::isPopup() { return false; }

QImage Mpris::getIcon() { return QImage(":/mpris/menu_icon.png"); }

QObject *Mpris::getQObject() { return this; }

QString Mpris::getName() { return tr("Media Player"); }
...
void Mpris::loadImage(QNetworkReply *reply) {
  ...
  if (!url.isEmpty()) {
    ...
    qDebug() << "Error in" << qPrintable(reply->url().toEncoded().constData())
             << ":" << reply->error() << " - "
