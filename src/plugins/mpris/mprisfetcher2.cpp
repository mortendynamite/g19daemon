...
void MprisFetcher2::connectToBus() {
  QDBusConnection::sessionBus().connect(
      "org.mpris.MediaPlayer2." + FPlayerName, "/org/mpris/MediaPlayer2",
...
void MprisFetcher2::updateStatus() {
  if (FPlayerInterface && FPlayerInterface->isValid()) {
    ...
    QDBusReply<QVariant> trackInfo =
        interface.call("Get", "org.mpris.MediaPlayer2.Player", "Metadata");

    if (trackInfo.isValid()) {
      QDBusArgument argument =
          static_cast<QVariant>(trackInfo.value()).value<QDBusArgument>();
      parseTrackInfo(qdbus_cast<QVariantMap>(argument));
    }

    QDBusReply<QVariant> playbackStatus = interface.call(
...
long MprisFetcher2::getPlayerPosition() {
  if (FPlayerInterface && FPlayerInterface->isValid()) {
    ...
    QDBusReply<QVariant> reply =
        interface.call("Get", "org.mpris.MediaPlayer2.Player", "Position");
...
void MprisFetcher2::onPropertyChange(QDBusMessage AMsg) {
  ...
  if (map.contains(QLatin1String("PlaybackStatus"))) {
    const QString sStatus =
        map.value(QLatin1String("PlaybackStatus")).toString();
    parsePlaybackStatus(sStatus);
  }
}

void MprisFetcher2::onPlayersExistenceChanged(QString AName, QString /*empty*/,
                                              QString ANewOwner) {
